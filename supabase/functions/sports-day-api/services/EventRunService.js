...
import { TABLES, EVENT_STATUS } from '../constants.js';
/** Create the EventRunService adapter with request-scoped dependencies. */
export function createEventRunService({
  Database,
  ServiceUtilities,
  LockService,
  services,
  ...
  const EventRunService = {
    ...
    updateEventMirror(eventIdentifier, status) {
      if (
        !Database.update(TABLES.EVENTS, eventIdentifier, { Status: status })
      ) {
